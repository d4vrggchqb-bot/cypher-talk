...
// Main page matrix rain effect
const mainCanvas = document.getElementById('matrix-bg');
const mainCtx = mainCanvas.getContext('2d');

mainCanvas.width = window.innerWidth;
mainCanvas.height = window.innerHeight;

const mainChars = 'ｱｲｳｴｵｶｷｸｹｺｻｼｽｾｿﾀﾁﾂﾃﾄﾅﾆﾇﾈﾉﾊﾋﾌﾍﾎﾏﾐﾑﾒﾓﾔﾕﾖﾗﾘﾙﾚﾛﾜﾝ0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
const mainFontSize = 16;
const mainColumns = Math.floor(mainCanvas.width / mainFontSize);
const mainDrops = Array(mainColumns).fill(1);
...
function drawMainMatrix() {
  ...
  mainCtx.fillStyle = '#00ff9f'; // Green color matching your theme
  mainCtx.font = mainFontSize + 'px monospace';

  for (let i = 0; i < mainDrops.length; i++) {
    // Random character from the set
    const text = mainChars[Math.floor(Math.random() * mainChars.length)];

    // Draw the character
    mainCtx.fillText(text, i * mainFontSize, mainDrops[i] * mainFontSize);

    // Reset drop if it reaches bottom with some randomness
    if (mainDrops[i] * mainFontSize > mainCanvas.height && Math.random() > 0.975) {
      mainDrops[i] = 0;
    }
...
let matrixInterval = setInterval(drawMainMatrix, 33);
...
window.addEventListener('resize', () => {
  ...
  const newColumns = Math.floor(mainCanvas.width / mainFontSize);
  mainDrops.length = newColumns;
  for (let i = 0; i < newColumns; i++) {
    mainDrops[i] = mainDrops[i] || 1;
  }
});

// Add to hideLoadingScreen function
const originalHideLoadingScreen = window.hideLoadingScreen || function () { };
window.hideLoadingScreen = function () {
  originalHideLoadingScreen();
  // Ensure matrix effect is running after loading screen disappears
  if (matrixInterval) clearInterval(matrixInterval);
  matrixInterval = setInterval(drawMainMatrix, 33);
};
...
function setupEventListeners() {
  ...
  window.logout = function () {
    ...
    window.location.href = '/login-page/login-page.html'; // Changed from '../'
  };
...
// Add subtle random flicker control
document.addEventListener('DOMContentLoaded', function () {
  const logos = document.querySelectorAll('.logo-title, .logo-image, .loading-logo');

  logos.forEach(logo => {
    // Randomly adjust animation intensity
    const randomDelay = Math.random() * 2;
    logo.style.animationDelay = `-${randomDelay}s`;

    // Occasionally trigger a stronger flicker
    setInterval(() => {
      if (Math.random() > 0.8) { // 20% chance every interval
        // Quick intense flicker
        const originalFilter = logo.style.filter;

        // Cyan burst
        logo.style.filter =
          'drop-shadow(0 0 25px #00ccff) ' +
          'drop-shadow(0 0 50px rgba(0, 204, 255, 0.8)) ' +
          'drop-shadow(0 0 75px rgba(0, 255, 159, 0.3))';
        ...
        setTimeout(() => {
          ...
          logo.style.filter =
            ...
            'drop-shadow(0 0 60px rgba(0, 255, 159, 0.2))';

          setTimeout(() => {
            // Return to normal with green overshoot
            logo.style.filter =
              'drop-shadow(0 0 30px #00ff9f) ' +
              'drop-shadow(0 0 60px rgba(0, 255, 159, 0.9))';
...
// Add this function to control logo blinking
function setupLogoBlinkControl() {
  console.log('🔧 Setting up logo blink control...');

  const logo = document.querySelector('.logo-title');
  if (!logo) {
    console.log('❌ Logo not found, checking for other logo classes...');
    return;
  }

  // Store original animation
  logo.dataset.originalAnimation = getComputedStyle(logo).animation;
  ...
  document.addEventListener('click', function (e) {
    ...
    if (conversationItem) {
      console.log('👤 User chatbox clicked, stopping logo blink...');

      // Pause the blinking animation on all logos
      const allLogos = document.querySelectorAll('.logo-title, .flash-container h1 .logo-image, .loading-logo');

      allLogos.forEach(logo => {
        // Store current filter before pausing
        if (!logo.dataset.pausedFilter) {
          logo.dataset.pausedFilter = getComputedStyle(logo).filter;
        }

        // Pause animation
        logo.style.animationPlayState = 'paused';

        // Set to a solid, non-blinking state
        // Get the current computed filter and lock it
        const currentFilter = getComputedStyle(logo).filter;

        // Extract color from filter or use default
        let finalColor = '#00ff9f'; // Default green

        // Try to detect current color from filter
        const filterMatch = currentFilter.match(/#[0-9a-fA-F]{6}/);
        if (filterMatch && filterMatch[0]) {
          finalColor = filterMatch[0];
        }

        // Apply solid glow with the detected color
        if (finalColor === '#00ff9f') {
          logo.style.filter =
            `drop-shadow(0 0 20px ${finalColor}) 
                         drop-shadow(0 0 40px rgba(0, 255, 159, 0.7))
                         drop-shadow(0 0 60px rgba(0, 255, 159, 0.4))
                         brightness(1.2)`;
        } else if (finalColor === '#ff00ff') {
          logo.style.filter =
            `drop-shadow(0 0 20px ${finalColor}) 
        ...
        // Add a 'paused' class for CSS targeting
        logo.classList.add('blink-paused');
        logo.classList.remove('blink-resumed');

        console.log(`✅ Logo blinking paused. Current color: ${finalColor}`);
      });
    }
  });

  // Optional: Resume blinking when clicking outside conversation items
  document.addEventListener('click', function (e) {
    // Check if click is NOT on a conversation item
    const conversationItem = e.target.closest('.conversation-item');
    const isLogoClick = e.target.closest('.logo-title, .flash-container h1 .logo-image, .loading-logo');

    if (!conversationItem && !isLogoClick) {
      const allLogos = document.querySelectorAll('.logo-title, .flash-container h1 .logo-image, .loading-logo');
      let shouldResume = false;

      // Check if any logo is paused
      allLogos.forEach(logo => {
        if (logo.style.animationPlayState === 'paused' ||
          logo.classList.contains('blink-paused')) {
          shouldResume = true;
        }
      });

      if (shouldResume) {
        console.log('🔄 Resuming logo blink...');

        allLogos.forEach(logo => {
          // Resume animation
          logo.style.animationPlayState = 'running';
          logo.style.filter = ''; // Reset to CSS animation

          // Update classes
          logo.classList.remove('blink-paused');
          logo.classList.add('blink-resumed');
  ...
  // Resume blinking when clicking on the logo itself
  document.addEventListener('click', function (e) {
    const logo = e.target.closest('.logo-title, .flash-container h1 .logo-image, .loading-logo');

    if (logo && (logo.style.animationPlayState === 'paused' ||
      logo.classList.contains('blink-paused'))) {
      console.log('🎬 Logo clicked, resuming blink...');
      ...
      allLogos.forEach(l => {
        ...
        l.style.filter = '';
        l.classList.remove('blink-paused');
        l.classList.add('blink-resumed');
  ...
  console.log('✅ Logo blink control setup complete');
}

// Call this function after DOM is loaded
document.addEventListener('DOMContentLoaded', function () {
  setTimeout(setupLogoBlinkControl, 1000);
});

// Also call when users are loaded (since conversation items are dynamic)
window.addEventListener('usersLoaded', setupLogoBlinkControl);
